...
class RNAPolytope(Polyhedron_QQ_ppl):
    def __init__(self, points, seq=None):
        """
    ...
    def construct_from_file(cls, polyfile):
        ...
        file_dir = os.path.dirname(polyfile)
        data_dir = os.path.join(file_dir, "..")
        sobj_dir = os.path.join(data_dir, "sobj")
        rnapoly_dir = os.path.join(data_dir, "rnapoly")
        filename = os.path.split(polyfile)[1]
        filename = os.path.splitext(filename)[0]
        full_sobj_path = os.path.join(sobj_dir, filename + ".sobj")
        full_rnapoly_path = os.path.join(rnapoly_dir, filename + ".rnapoly")
        print(full_rnapoly_path)

        # Construct the polytope
        try:
            # If the polytope is available in a pickle, we should use that
            thepoly = load(full_sobj_path) # Attempt to load the polytope from a pickle file
            ...
            points = []
            new_seq = ""
            with open(full_rnapoly_path) as f:
                for line in f:
                    if new_seq == ""  and line.startswith("#\t") :#if we have not found the sequence yet and we are looking at a line beginning with # followed by tab
                        new_seq = line.split()[1] #save the sequence so we can put it in the object later
            ...
            thepoly = RNAPolytope(points, seq=new_seq)
            thepoly._pickle_version = pickle_version
            thepoly._build_normal_fan()
            thepoly._build_d1_slices()
            thepoly.dump(full_sobj_path, -1)
